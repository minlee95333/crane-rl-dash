...
_PYTORCH_POLICY_NAMES = {"mappo": "PyTorch MAPPO", "nearest": "Nearest", "radiusPriority": "Same-radius-priority", "random": "Random"}


def _norm(doc: Dict[str, Any], split: str, policy: str, engine: str, rename_policy: bool = False) -> Dict[str, Any]:
    """Flatten one (split, policy) slice of a result document into a row.
    Single source of truth for the field set; previously two near-identical helpers
    drifted (browser variant used `src.get('completeRate', src.get('completion', 0))`,
    pytorch did not — that fallback is preserved here for backward compatibility)."""
    src = doc.get(split, {}).get(policy, {})
    name = _PYTORCH_POLICY_NAMES.get(policy, policy) if rename_policy else policy
    return {
        "engine": engine,
        "split": split,
        "policy": name,
...
def main():
    ...
    browser_missing = not browser_path.exists()
    browser = {} if browser_missing else load_json(browser_path)
    ...
    for split in ["seen", "unseen"]:
        for pol in browser_policies:
            if pol in browser.get(split, {}):
                rows.append(_norm(browser, split, pol, engine="browser-light"))
    ...
    if browser_missing:
        notes.append(f"Browser-light 입력 파일이 없어 PyTorch 결과만 포함했다: {browser_path}")
    comparison = {
        "notes": notes,
