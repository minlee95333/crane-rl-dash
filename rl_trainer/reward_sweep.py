...
try:
    from .train import load_cfg, train, pct_improve
except ImportError:
    from train import load_cfg, train, pct_improve
    ...
    for p_inter in parse_floats(args.p_inter_soft):
        for p_move in parse_floats(args.p_move):
            ...
            row = {
                ...
                'seenImproveVsNearestPct': pct_improve(n_seen.get('makespan', 0), m_seen.get('makespan', 0)) if n_seen.get('makespan') else None,
                'seenSoft': m_seen.get('soft'),
                'seenHardExecuted': m_seen.get('hardExecuted'),
                'seenHardMask': m_seen.get('hardMask'),
                'seenMove': m_seen.get('move'),
                'seenTeardown': m_seen.get('teardown'),
                'unseenMakespan': m_unseen.get('makespan'),
                'unseenNearestMakespan': n_unseen.get('makespan'),
                'unseenImproveVsNearestPct': pct_improve(n_unseen.get('makespan', 0), m_unseen.get('makespan', 0)) if n_unseen.get('makespan') else None,
                ...
                'unseenTeardown': m_unseen.get('teardown'),
            }
            rows.append(row)
